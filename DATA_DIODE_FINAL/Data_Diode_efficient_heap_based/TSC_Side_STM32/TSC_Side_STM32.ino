...
void loop() {
  ...
  if (curFrame == NULL && !pending.isEmpty()) {
    ...
    //Uncomment this code to view encoded SPaT Data
...
void sendDiode(struct Frame *frame) {
  ...
  if (allowed > 0) {
    ...
    DEBUG_PRINT("len=%d frame->length=%d frame->pos=%d\n",len,frame->length,frame->pos);
...
void handleUDP() {
  ...
  //Uncomment this code to view the raw SPaT data;cycle hungry code. can be bettered
  
  /*
  DEBUG_PRINT("udp spat data : ");
  ...
  */
  ...
  crc.restart();
  crc.add(data, pktLen);
  uint32_t crc32 = crc.calc();

  // Add CRC32 to data
  //note that the memcpy is little endian i.e. if crc=0xDEAF1234 and data=0xFEED6789 then after memcpy the data will be 0xFEED67893412AFDE This needs to handled at the python side
